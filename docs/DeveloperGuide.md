...
This project securely relies on lockfiles to guarantee **Build Reproducibility** across different developers and CI environments. There are two primary lockfiles tracked in this repository:
1. **`dependencies.lock`** (Firmware/ESP-IDF): Automatically generated by PlatformIO. Determines the exact versions of the C++ components and library trees built into the ESP32 firmware.
2. **`test/web/package-lock.json`** (Web Portal Testing): Automatically generated by NPM for testing frameworks.

### Best Practices
* **Never add lockfiles to `.gitignore`**. They must always be tracked and committed to version control.
* **Never manually edit lockfiles**. They are machine-generated.

### The Dependency Bump Chore
To ensure the project receives important security and bugfix patches without unexpectedly breaking your daily feature development, you should periodically perform a controlled "Dependency Bump Chore". This is standard proactive maintenance, recommended ahead of major release milestones.

**Workflow for the Update Chore:**
1. Create a dedicated update branch: `git checkout -b chore/update-deps`
2. Force the build system to fetch the absolute latest allowed packages and rewrite the `dependencies.lock` file:
   ```bash
   pio pkg update
   ```
3. (Optional) Force the Node.js packages to update:
   ```bash
   cd test/web && npm update
   ```
4. Rebuild the firmware and run your standard hardware tests to ensure no upstream changes have broken the project.
5. Formalize it by committing the newly generated `.lock` files:
   ```bash
   git add dependencies.lock test/web/package-lock.json
   git commit -m "chore: intentionally bump and lock latest dependencies"
   ```
