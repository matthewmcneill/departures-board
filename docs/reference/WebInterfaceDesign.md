...
- **Unified Test Queue**: All diagnostic tests (Board Health, API Keys, Weather, and Feeds) are enqueued in a global `app.state.testQueue`.
- **Staggered Requests**: Tests are executed strictly one-by-one with a **500ms breather** between each request to allow the ESP32's WiFi and LwIP stacks to process other background tasks.
- **Auto-Execution**: Sequential tests are triggered automatically whenever a relevant tab (Displays, Keys, Feeds) is loaded or active data is modified.
- **Automatic Abort**: Switching tabs immediately clears the test queue to focus hardware resources on the current user context.
