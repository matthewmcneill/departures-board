...
#define BUS_MAX_LOCATION 45 // Maximum string length for destination name
#define BUS_MAX_LINE 16     // Maximum string length for line identifier
#define BUS_MAX_SERVICES 9  // Maximum number of services to render on board

#define BUS_MAX_LINE_NAME 9 // Maximum string length for a bus route name
#define BUS_MAX_FETCH 20    // Maximum number of services to parse from HTML
#define BUS_MAX_FILTER 25   // Maximum string length for the user route filter

// HTML Scraper parsing states
#define PBT_START 0       // Initial parser state
#define PBT_HEADER 1      // Parsing the departures table header
#define PBT_SERVICE 2     // Parsing a bus departure row
#define PBT_DESTINATION 3 // Extracting the destination name column
#define PBT_SCHEDULED 4   // Extracting the scheduled time column
#define PBT_EXPECTED 5    // Extracting the expected real-time column
...
class busDataClient: public JsonListener {
    ...
    private:
        ...
        struct busService {
            char destinationName[BUS_MAX_LOCATION];
            char lineName[BUS_MAX_LINE_NAME];
            char scheduled[6];
            char expected[6];
        };
        ...
        struct busStop {
            int numServices;
            busService service[BUS_MAX_FETCH];
        };
    ...
    public:
        ...
        * @param station Pointer to the BusStop structure to populate with arrival times.
        ...
        */
        int updateDepartures(BusStop *station, const char *locationId, const char *filter, busClientCallback Xcb);
