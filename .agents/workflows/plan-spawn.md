...
4. **⚠️ Portability & Path Handling**:
    - Use repository-relative paths for all codebase, script, or `.agents/plans/` references.
    - Use absolute paths **ONLY** when referring to the session's `/brain` directory.
5. **Mandatory Planning Step**: You **MUST** include an explicit instruction requiring an implementation plan. Use wording similar to:
