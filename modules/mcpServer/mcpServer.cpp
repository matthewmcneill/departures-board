 ...
 * Description: Implementation of the MCP JSON-RPC execution logic independently of any HTTP web handlers.
 *
 * Exported Functions/Classes:
 * - mcpServer::init(): Initializes the tool mappings.
 * - mcpServer::processPayload(): Evaluates the requested method against registered tools and dispatches them.
 */
...
const char tools_list_response[] PROGMEM = 
    ...
    "{\"name\":\"get_system_telemetry\",\"description\":\"Get hardware telemetry (memory, temp, uptime)\",\"inputSchema\":{\"type\":\"object\",\"properties\":{}}},"
...
namespace mcpServer {
    ...
    /**
     * @brief Encodes raw byte array to Base64 in small chunks directly to Print& to avoid massive heap allocations
     */
    void printChunkedBase64(const uint8_t* data, size_t len, Print& out) {
        size_t offset = 0;
        while(offset < len) {
            size_t chunk = (len - offset > 48) ? 48 : (len - offset); // 48 divides by 3 cleanly
    ...
    void processPayload(const JsonDocument& requestDoc, Print& outputStream) {
        ...
        if (method == "") {
            outputStream.print("{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32600,\"message\":\"Invalid Request\"}}");
            return;
        }
        ...
        if (method == "tools/list") {
            ...
            outputStream.print("{\"jsonrpc\":\"2.0\",\"error\":{\"code\":-32601,\"message\":\"Method not found\"}}");
        }
    }
