 ...
 * iNationalRailDataProvider.
 *   - init(): Attempts discovery initialization for SOAP endpoints via cached
 * parameters.
 *   - refreshWsdl(): SOAP discovery and WSDL parsing over HTTPS.
 *   - updateData(): Intercept and request a priority update from the data
 * manager.
 *   - configure(): Bootstrapping routine to sink configuration parameters into
 * instance.
 *   - executeFetch(): Internal blocking HTTP REST/SOAP engine for retrieving
 * and parsing XML payload.
 *   - testConnection(): Validates station codes and tokens using lightweight
 * query.
 */
...
nrDARWINDataProvider::nrDARWINDataProvider()
    : loadingWDSL(false), tagLevel(0), isTestMode(false), id(-1), coaches(0),
      addedStopLocation(false), filterPlatforms(false), keepRoute(false),
      nrTimeOffset(0), messagesData(4), renderMessages(4),
 ...
 */
void nrDARWINDataProvider::configure(const char *token, const char *crs,
 ...
 */
bool nrDARWINDataProvider::compareTimes(const NationalRailService &a,
                                        const NationalRailService &b) {
  int h1, m1, h2, m2;
 ...
 * Attempts to load cached endpoints from LittleFS; falls back to WSDL
 * discovery.
 * @return Success if endpoints are valid.
 */
UpdateStatus nrDARWINDataProvider::init(const char *wsdlHost,
                                        const char *wsdlAPI) {

  // Attempt to load discovered SOAP endpoints from dedicated cache file
  if (LittleFS.exists(F("/darwin_wsdl_cache.json"))) {
    String contents =
        appContext.getConfigManager().loadFile(F("/darwin_wsdl_cache.json"));
    JsonDocument doc;
    DeserializationError error = deserializeJson(doc, contents);
    if (!error && doc[F("host")].is<const char *>() &&
        doc[F("api")].is<const char *>()) {
      strlcpy(soapHost, doc[F("host")], sizeof(soapHost));
      strlcpy(soapAPI, doc[F("api")], sizeof(soapAPI));
      LOG_INFO("DATA", "NR Source: Initialized from standalone cache: " +
                           String(soapHost) + String(soapAPI));
  ...
  LOG_INFO(
      "DATA",
      "NR Source: No valid cache found. Triggering first-boot discovery...");
  return refreshWsdl();
}

void nrDARWINDataProvider::setSoapAddress(const char *host, const char *api) {
  if (host)
    strlcpy(soapHost, host, sizeof(soapHost));
  if (api)
    strlcpy(soapAPI, api, sizeof(soapAPI));
  isTestMode = true;
  LOG_INFO("DATA", "NR Source: Soap address manually set to " +
                       String(soapHost) + String(soapAPI) +
                       " (Test Mode enabled)");
}
...
UpdateStatus nrDARWINDataProvider::updateData() {
  ...
  // Intercept completion from background task, acknowledge it to the UI, and
  // rest.
...
PriorityTier nrDARWINDataProvider::getPriorityTier() {
  // If we've never successfully loaded data, it's critically empty -> High
  // Priority
...
/**
 * @brief Internal blocking method that executes the SOAP protocol and
...
void nrDARWINDataProvider::executeFetch() {
  ...
  // --- Self-Initialization ---
  if (!isInitialized()) {
    LOG_INFO("DATA", "NR Source: Data source uninitialized. Running deferred "
                     "init on background task...");
    UpdateStatus initStatus = init("lite.realtime.nationalrail.co.uk",
  ...
  if (!httpsClient) {
    LOG_ERROR("DATA",
              "NR Source: Failed to allocate WiFiClientSecure for update!");
  ...
  while (!httpsClient->connect(soapHost, 443) && soapRetry < 5) {
    LOG_WARN("DATA", String("NR Source: Connection attempt ") + soapRetry +
                         " failed. Retrying...");
    delay(300);
    soapRetry++;
  }
  ...
  LOG_INFO("DATA", "NR Source: Connection established. Free heap: " +
                       String(ESP.getFreeHeap()));

  // Allocate temporary stations on the heap AFTER SSL link is up to conserve
  // peak heap
  ...
  if (!isTestMode) {
    ...
    if (!xStation) {
      LOG_ERROR(
          "DATA",
          "NR Source: Failed to allocate memory for station data update!");
      httpsClient->stop();
      taskStatus = UpdateStatus::DATA_ERROR;
      return;
    }
  ...
  if (isTestMode) {
    ...
    data += F("<Header><AccessToken "
              "xmlns=\"http://thalesgroup.com/RTTI/2013-11-28/Token/"
              "types\"><TokenValue>");
    data += String(nrToken) + F("</TokenValue></AccessToken></Header>");
    data += F("<Body><GetDepartureBoardRequest "
              "xmlns=\"http://thalesgroup.com/RTTI/2021-11-01/ldb/"
              "\"><numRows>1</numRows><crs>");
    data += String(crsCode) +
            F("</crs></GetDepartureBoardRequest></Body></Envelope>");
    soapAction =
        F("http://thalesgroup.com/RTTI/2012-01-13/ldb/GetDepartureBoard");
    ...
    data = F("<soap-env:Envelope "
             "xmlns:soap-env=\"http://schemas.xmlsoap.org/soap/envelope/\" "
             "xmlns:ns1=\"http://thalesgroup.com/RTTI/2013-11-28/Token/types\" "
             "xmlns:ns2=\"http://thalesgroup.com/RTTI/2021-11-01/ldb/\">");
    data += F("<soap-env:Header><ns1:AccessToken><ns1:TokenValue>");
    data += String(nrToken) + F("</ns1:TokenValue></ns1:AccessToken></"
                                "soap-env:Header><soap-env:Body><ns2:"
                                "GetDepBoardWithDetailsRequest><ns2:numRows>");
    data += String(reqRows) + F("</ns2:numRows><ns2:crs>");
    data += String(crsCode) + F("</ns2:crs>");
    if (callingCrsCode[0])
      data += "<ns2:filterCrs>" + String(callingCrsCode) +
              F("</ns2:filterCrs><ns2:filterType>to</ns2:filterType>");
    if (nrTimeOffset)
      data +=
          "<ns2:timeOffset>" + String(nrTimeOffset) + F("</ns2:timeOffset>");
    data += F("</ns2:GetDepBoardWithDetailsRequest></soap-env:Body></"
              "soap-env:Envelope>");

    soapAction =
        F("http://thalesgroup.com/RTTI/2015-05-14/ldb/GetDepBoardWithDetails");
  }
  ...
  if (strlen(nrToken) > 0) {
    ...
    LOG_INFO("DATA", "NR Source: Sending request with token: " + preview +
                         " (Len: " + String(t.length()) + ")");
  } else {
  ...
  if (tokenIdx >= 0) {
    debugReq = debugReq.substring(0, tokenIdx) + "********" +
               debugReq.substring(tokenIdx + strlen(nrToken));
  }
  ...
  int retry = 0;
  while (!httpsClient->available() && retry < 80) {
    delay(100);
    retry++;
  }
  if (retry >= 80) {
  ...
  while (httpsClient->connected() || httpsClient->available()) {
    ...
    if (line.startsWith("HTTP")) {
      ...
      if (line.indexOf("200 OK") == -1) {
        ...
        snprintf(lastErrorMessage, sizeof(lastErrorMessage), "HTTP Error: %s",
                 line.c_str());
        ...
        if (line.indexOf("404") != -1 || line.indexOf("500") != -1) {
          LOG_WARN("DATA", "NR Source: Endpoint returned fatal error. "
                           "Triggering WSDL refresh...");
          refreshWsdl();
        }
        ...
        LOG_ERROR("DATA",
                  "NR Source: Error Body: " + faultBody.substring(0, 300));
        ...
        LOG_INFO("DATA",
                 "NR Source: Test Mode validated HTTP 200 via fast-exit.");
        httpsClient->stop();
  ...
  while (httpsClient->available() || httpsClient->connected()) {
    ...
    if (avail > 0) {
      ...
      if (len > 0) {
        ...
        String rawDump = "";
        for (int j = 0; j < len; j++)
          rawDump += (char)buffer[j];
        LOG_VERBOSE("DATA-RAW", rawDump);
#endif
        for (int i = 0; i < len; i++) {
          ...
          // --- Arcane Logic ---
          // On single-core ESP32 variants (e.g. ESP32-C3), the Wi-Fi stack and
          // user application share the exact same processor core tightly via
          // the RTOS scheduler. By explicitly yielding execution context via
          // vTaskDelay(1) every 500 byte blocks, we guarantee network hardware
          // interrupts service without triggering Task Watchdog Timers (TWDT).
          yieldCounter++;
          if (yieldCounter % 500 == 0)
            vTaskDelay(1); // Single-core compatibility yield
  ...
  httpsClient->stop();
  LOG_INFO("DATA",
           "NR Source: XML parse complete. Bytes: " + String(bytesRecv));

  // After parsing, sanitize and check changes
  sanitiseData();

  // For now, always treat a successful parse as a change to trigger board
  // refresh Simplifies memory and is generally what's needed for departures
  if (stationData) {
  ...
  if (renderData) { // Removed callback trigger
    snprintf(lastErrorMessage, sizeof(lastErrorMessage), "SUCCESS [%ld] %lums",
             bytesRecv, (unsigned long)(millis() - perfTimer));
    ...
    if (renderData && renderData->numServices > 0) {
      ...
      // We could parse renderData->service[0].sTime or etd to find exactly when
      // to wake up. For now, if we have services, we enforce a baseline wait of
      // 60 seconds to conserve API limits.
      interval = 60000;
    }
    ...
    if (renderData) {
      ...
      for (int i = 0; i < renderData->numServices; i++) {
        ...
        snprintf(debugMsg, sizeof(debugMsg),
                 "Service %d: %s to %s (Exp: %s, Plat: %s)", i,
                 renderData->service[i].sTime,
                 renderData->service[i].destination, renderData->service[i].etd,
                 renderData->service[i].platform);
...
/**
 * @brief Performs a lightweight connection and authentication test using a
 * minimal payload.
 * @param token Optional token to test (overrides stored configuration).
 * @return UpdateStatus code.
 */
UpdateStatus nrDARWINDataProvider::testConnection(const char *token,
                                                  const char *stationId) {
  LOG_INFO(
      "DATA",
      "NR Source: Performing lightweight Auth-only check via testConnection");
  ...
  if (stationId && strlen(stationId) > 0) {
    ...
    strlcpy(crsCode, "PAD",
            sizeof(crsCode)); // Paddington as default test station
  }
  ...
  // Convert UPD_NO_CHANGE (1) to UpdateStatus::SUCCESS (0) for test result
  // consistency since we don't care about board state changing
  if (result == UpdateStatus::NO_CHANGE)
...
void nrDARWINDataProvider::sanitiseData() {
  ...
  // Note: Sanitization for MessagePool strings happens during addition or
  // after? Current Darwin logic adds them. We'll leave them as is for now or
  // add a sanitize method to Pool.
}
...
void nrDARWINDataProvider::replaceWord(char *input, const char *target,
...
bool nrDARWINDataProvider::equalsIgnoreCase(const char *a, int a_len,
                                            const char *b) {
...
bool nrDARWINDataProvider::serviceMatchesFilter(const char *filter,
  ...
  while (true) {
    if (*ptr == ',' || *ptr == '\0') {
      ...
      trim(tS, tE);
      int len = tE - tS + 1;
      if (len > 0 && equalsIgnoreCase(tS, len, serviceId))
        return true;
      if (*ptr == '\0')
        break;
      start = ++ptr;
...
void nrDARWINDataProvider::deleteService(int x) {
  if (!stationData || x < 0 || x >= stationData->numServices)
    return;
  for (int i = x; i < stationData->numServices - 1; i++)
    stationData->service[i] = stationData->service[i + 1];
...
void nrDARWINDataProvider::value(const char *val) {
  ...
  if (tagPath.indexOf("callingPoint/lt8:locationName") != -1) {
    if (id == 0 && stationData &&
        (strlen(stationData->firstServiceCalling) + strlen(val) + 10) <
            NR_MAX_CALLING) {
      if (stationData->firstServiceCalling[0])
        strcat(stationData->firstServiceCalling, ", ");
      strcat(stationData->firstServiceCalling, val);
      addedStopLocation = true;
    }
  } else if (tagPath.indexOf("callingPoint/lt8:st") != -1 &&
             addedStopLocation) {
    if (id == 0 && stationData &&
        (strlen(stationData->firstServiceCalling) + strlen(val) + 4) <
            NR_MAX_CALLING) {
      strcat(stationData->firstServiceCalling, " (");
      strcat(stationData->firstServiceCalling, val);
      strcat(stationData->firstServiceCalling, ")");
    ...
    addedStopLocation = false;
  } else if (tagName == "lt4:operator" &&
             tagPath.indexOf("lt8:service/lt4:operator") != -1) {
    if (id >= 0 && stationData)
      strncpy(stationData->service[id].opco, val, 49);
  ...
  } else if (tagName == "lt4:etd" &&
             tagPath.indexOf("lt8:service/lt4:etd") != -1) {
    if (id >= 0 && stationData)
      strncpy(stationData->service[id].etd, val, 10);
  } else if (tagPath.indexOf("destination") != -1 &&
             tagPath.indexOf("locationName") != -1) {
    if (id >= 0 && stationData)
      strncpy(stationData->service[id].destination, val, NR_MAX_LOCATION - 1);
  } else if (tagPath.indexOf("destination") != -1 &&
             tagPath.indexOf("via") != -1) {
    if (id >= 0 && stationData)
      strncpy(stationData->service[id].via, val, NR_MAX_LOCATION - 1);
  } else if (tagName == "lt4:platform" &&
             tagPath.indexOf("lt8:service/lt4:platform") != -1) {
    if (id >= 0 && stationData) {
  ...
  } else if (tagName == "lt4:locationName" &&
             tagPath.indexOf("callingPoint") == -1 &&
             tagPath.indexOf("destination") == -1 &&
             tagPath.indexOf("origin") == -1) {
    if (stationData)
      strncpy(stationData->location, val, NR_MAX_LOCATION - 1);
  } else if (tagPath.indexOf("lt12:lastReportedStationName") != -1) {
    if (id == 0 && stationData)
      strncpy(stationData->firstServiceLastSeen, val, NR_MAX_LOCATION - 1);
...
void nrDARWINDataProvider::attribute(const char *attr) {
  if (loadingWDSL && tagName == "soap:address") {
    ...
    if (start != -1) {
      ...
      if (end != -1)
        soapURL = s.substring(start + 10, end);
    }
  }
}
...
UpdateStatus nrDARWINDataProvider::refreshWsdl() {
  LOG_INFO("DATA",
           "NR Source: Refreshing WSDL to discover latest SOAP endpoints...");
  ...
  if (!client->connect(host, 443)) {
    LOG_ERROR("DATA",
              "NR Source: Failed to connect to WSDL host: " + String(host));
  ...
  if (soapURL.length() > 0) {
    ...
    if (soapURL.startsWith("https://")) {
      ...
      if (slashIdx != -1) {
        ...
        LOG_INFO(
            "DATA",
            "NR Source: Archiving discovery to /darwin_wsdl_cache.json...");
        JsonDocument doc;
        doc[F("host")] = soapHost;
        doc[F("api")] = soapAPI;
        String output;
