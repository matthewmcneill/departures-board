 ...
 *              data refresh timers) extracted from Departures Board.cpp and
 * systemBoard.cpp.
 *
 * Exported Functions/Classes:
 * - systemManager: State container and business logic orchestrator for
 * non-display tasks.
 */
...
private:
  ...
  // --- Data Polling State ---
  unsigned long
      nextRoundRobinUpdate;  ///< Calculated next background poll timestamp (ms)
  int backgroundUpdateIndex; ///< Current index in the background fetch carousel
  unsigned long
      lastDataLoadTime; ///< Timestamp of last successful data load (ms)
...
public:
  ...
  void setBootProgressCallback(
      std::function<void(const char *msg, int percent)> cb) {
    onBootProgress = cb;
  }
  ...
  void setStartupProgressPercent(int percent) {
    startupProgressPercent = percent;
  }

  unsigned long getNextRoundRobinUpdate() const { return nextRoundRobinUpdate; }
  void setNextRoundRobinUpdate(unsigned long time) {
    nextRoundRobinUpdate = time;
  }
